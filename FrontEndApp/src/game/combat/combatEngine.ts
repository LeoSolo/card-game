...
const getModifierSum = (state: CombatState, type: EquipmentModifier['type']): number =>
...
const reduceTemporaryStatuses = (actor: CombatActor): void => {
  (['weak', 'vulnerable'] as const).forEach((status) => {
    if ((actor.statuses[status] ?? 0) > 0) {
  ...
  card.effects.forEach((effect) => {
    if (effect.type === 'damage') {
      if (effect.target === 'all-enemies') {
        getAliveEnemies(state).forEach((enemy) => dealDamage(state, state.player, enemy, effect.amount));
        return;
      }
...
const enemyTurn = (state: CombatState): void => {
  ...
  reduceTemporaryStatuses(state.player);
  state.turn += 1;
